...
namespace RouteScout.Web.Server.Integration.AddressWashing
{
    public class AddressConfirmedAddStopHandler : IAddressCandidateConfirmedHandler
    {
        ...
        public AddressConfirmedAddStopHandler(IDocumentStore store, IStopService stopService)
        {
            _store = store;
        ...
        public async Task HandleAsync(AddressConfirmed @event)
        {
            ...
            await _stopService.CreateStop(selectedAddress.Id, selectedAddress.StreetId, selectedAddress.Street, selectedAddress.Number, candidate.Amount);
        }
    }
}
